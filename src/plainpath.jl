...
```
"""
abstract type PlainPath{L} <: AbstractPath{SubString{String}, L} end
...
# Some nice general methods

Base.convert(::Type{String}, p::PlainPath) = String(p)

Base.:(==)(a::P, b::P) where {P <: PlainPath} = String(a) == String(b)

Base.hash(p::P, h::UInt) where {P <: PlainPath} = hash(String(p), hash(P, h))

function Base.startswith(a::P, b::P) where {P <: PlainPath}
    astr, bstr = String(a), String(b)
    if ncodeunits(astr) < ncodeunits(bstr)
        false
    elseif ncodeunits(astr) == ncodeunits(bstr)
        astr == bstr
    else
        startswith(astr, bstr) && codeunit(astr, ncodeunits(bstr) + 1) == separatorbyte(P)
    end
end

Base.:(<)(a::P, b::P) where {P <: PlainPath} = startswith(b, a)
Base.:(>)(a::P, b::P) where {P <: PlainPath} = startswith(a, b)

function Base.endswith(a::P, b::P) where {P <: PlainPath}
    astr, bstr = String(a), String(b)
    if ncodeunits(astr) < ncodeunits(bstr)
        false
    elseif ncodeunits(astr) == ncodeunits(bstr)
        ...
        endswith(astr, bstr) && codeunit(astr, ncodeunits(astr) - ncodeunits(bstr)) == separatorbyte(P)
    end
end
